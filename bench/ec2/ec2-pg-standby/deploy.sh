...
log "primary: $SRC_PRIV:5432   standby image: $PG_IMAGE"
...
log "taking base backup from primary (this can take a moment)"
remote_sh "$PG_IMAGE" "$SRC_PRIV" <<'EOS'
set -e
image="$1" src_host="$2"
...
sudo docker run --rm --user postgres -v standby-data:/var/lib/postgresql/data "$image" \
  pg_basebackup -h "$src_host" -p 5432 -U postgres -D /var/lib/postgresql/data -R -X stream -c fast -P
EOS
...
log "starting streaming standby (read-only, :5432)"
remote_sh "$PG_IMAGE" <<'EOS'
sudo docker run -d --name pg-standby --restart unless-stopped \
  -p 5432:5432 \
  -v standby-data:/var/lib/postgresql/data \
  "$1" postgres -c listen_addresses='*' >/dev/null && echo started
EOS
