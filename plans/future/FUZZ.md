...
wal_rs = { package = "wal-rs", path = "../wal-rs" }
...
|---|---|---|---|
| `wal_parse_record` | `wal_rs::pg::walparser::parse_record_from_bytes(data, page_magic)` | wal-rs | HIGH, core XLogRecord header + block headers + FPI metadata |
| `wal_extract_locations` | `wal_rs::pg::walparser::extract_block_locations` / `extract_locations_from_wal_file<R: Read>` | wal-rs | HIGH, full-segment walk, integrates lower parsers |
| `wal_decode_frame` | `wal_rs::pg::replication::stream::decode_frame(payload)` | wal-rs | MED-HIGH, network-facing `'w'`/`'k'` CopyData frames |
| `daemon_parse_args` | `wal_rs::daemon::protocol::parse_args(body)` | wal-rs | MED, length-prefixed arg vector |
...
- **`wal_parser_stateful`** — `wal_rs::pg::walparser::WalParser::parse_records_from_page`
...
- **Re-exported paths only.** `parse_record_from_bytes` and
  `extract_block_locations` are re-exported from `wal_rs::pg::walparser`;
