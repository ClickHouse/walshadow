...
//! ([`crate::visibility`]) the greenfield walk lacks. Nothing lands on disk:
...
use crate::backup_source::{BackupSink, BackupSource};
use crate::backup_source_direct::DirectSource;
use crate::backup_source_object_store::{ObjectStoreSource, build_lsn_pair};
use crate::ch_emitter::{EmitterConfig, EmitterStats, MappingHandle};
use crate::heap_decoder::{XLOG_HEAP_OPMASK, XLOG_HEAP_TRUNCATE};
use crate::manifest::Manifest;
use crate::pg_class_decoder::{DecodeOutcome, decode_pg_class_tuple, info_carries_new_tuple_heap};
...
use crate::wal_stream::{Record, RecordSink, SegmentSink, SinkError, WAL_SEG_SIZE, WalStream};
use crate::xact_buffer::{
    BufferingDecoderSink, SubxactTracker, XactBuffer, XactBufferConfig, XactRecordSink,
    parse_xact_payload,
};

/// One opted-in rel awaiting backup-sourced backfill; `s_lsn` is the opt-in
/// boundary `S` recorded in the ledger.
#[derive(Debug, Clone)]
...
/// Everything a pass borrows from the daemon. One struct so the coalescing
/// dispatcher in [`crate::copy_backfill`] hands it through unchanged.
...
impl PrescanSink {
    fn observe(&mut self, record: &Record<'_>) {
        ...
        if rm == RmId::Xact as u8 {
            ...
            match info & crate::xact_buffer::XLOG_XACT_OPMASK {
                crate::xact_buffer::XLOG_XACT_COMMIT
                | crate::xact_buffer::XLOG_XACT_COMMIT_PREPARED => {
                    let payload = parse_xact_payload(info, &record.parsed.main_data);
                    self.patch.commit(xid, &payload.subxacts);
                }
                crate::xact_buffer::XLOG_XACT_ABORT
                | crate::xact_buffer::XLOG_XACT_ABORT_PREPARED => {
                    let payload = parse_xact_payload(info, &record.parsed.main_data);
...
/// [`crate::decoder_sink::TupleObserver`] shipping gap-committed rows onto
...
impl crate::decoder_sink::TupleObserver for ReplayRouter {
    fn on_tuple<'a>(
        &'a mut self,
        committed: &'a crate::heap_decoder::CommittedTuple,
    ) -> Pin<
        Box<
            dyn Future<Output = std::result::Result<(), crate::decoder_sink::DecoderSinkError>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            ...
            self.msg_tx
                ...
                .await
                .map_err(|_| {
                    crate::decoder_sink::DecoderSinkError::Observer(
    ...
    fn on_xact_end<'a>(
        ...
        Box<
            dyn Future<Output = std::result::Result<u64, crate::decoder_sink::DecoderSinkError>>
                + Send
                + 'a,
        >,
    > {
...
mod tests {
    ...
    fn prescan_harvests_commit_and_abort_into_patch() {
        ...
        assert_eq!(
            view.xid_status(700),
            crate::visibility::XidStatus::Committed
        );
        assert_eq!(view.xid_status(701), crate::visibility::XidStatus::Aborted);
    }
