//! Phase 14 — `kill -9` mid-stream + restart drill (v1.0 acceptance §5).
//!
...
//!      walsender bind (Phase 13 SO_REUSEADDR), no `--ignore-cursor`
...
fn append_source_conf(sh: &Shadow) -> Result<()> {
    ...
    writeln!(f, "\n# walshadow phase14 kill-restart source overrides")?;
...
fn rewrite_for_shadow(data_dir: &Path, port: u16, socket_dir: &Path) -> Result<()> {
    ...
    writeln!(f, "\n# walshadow phase14 kill-restart shadow overrides")?;
    writeln!(f, "port = {port}")?;
...
fn enable_recovery(data_dir: &Path, restore_from: &Path, walsender_port: u16) -> Result<()> {
    ...
    let conf = data_dir.join("postgresql.conf");
    let mut f = fs::OpenOptions::new().append(true).open(&conf)?;
    writeln!(f, "\n# walshadow phase14 kill-restart recovery")?;
    writeln!(
        f,
...
async fn run_cycle(
    ...
    eprintln!(
        "phase14-kill-restart: cycle start strategy={strategy:?} run={run} kill_delay={kill_delay_ms}ms",
    );
    ...
    eprintln!(
        "phase14-kill-restart: post-kill source lsn={:X}/{:X}",
        post_kill_lsn >> 32,
    ...
    eprintln!("phase14-kill-restart: cycle ok strategy={strategy:?} run={run}");
    Ok(())
}
