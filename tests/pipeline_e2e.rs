//! End-to-end DDL drill: source PG → walshadow filter →
...
// Non-overlapping ports — pipeline tests live in the 17100-range; pick
...
// Source needs wal_level=logical + max_wal_senders so
...
fn append_source_conf(sh: &Shadow) {
    ...
    writeln!(f, "\n# walshadow pipeline-e2e source overrides").unwrap();
...
fn rewrite_for_shadow(data_dir: &Path, port: u16, socket_dir: &Path) -> Result<()> {
    ...
    writeln!(f, "\n# walshadow pipeline-e2e shadow overrides")?;
    writeln!(f, "port = {port}")?;
...
// ---------------------------------------------------------------------------
// Bootstrap helper — shared by every pipeline-e2e test. Builds a source PG
...
async fn insert_update_delete_replicates_to_clickhouse() {
    if !pg_available() {
    ...
    let pgcfg = PgConfig {
        ...
        application_name: "walshadow-pipeline-e2e".into(),
    ...
    eprintln!(
        "pipeline-e2e: shipped {} segments, target_lsn={:X}/{:X}, decoder={}, xact_buffer={}",
        segments_shipped,
        target >> 32,
        target as u32,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn add_column_replicates_pre_and_post_alter() {
    if !pg_available() {
        eprintln!("skip: no initdb on PATH");
        return;
    }
    ...
    let pgcfg = PgConfig {
        ...
        password: None,
        database: "postgres".into(),
        application_name: "walshadow-pipeline-evo".into(),
        sslmode: SslMode::Disable,
    };
    let mut feed = SourceFeed::connect(&pgcfg)
        .await
    ...
    eprintln!(
        "pipeline-evo: shipped {} segments, decoder={}, xact_buffer={}",
        segments_shipped,
        record_sink.decoder.stats().summary(),
        xact_buffer.lock().await.stats().summary(),
    );
