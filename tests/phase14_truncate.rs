//! Phase 14 item 3 — `TRUNCATE` propagation.
//!
//! Verifies that source `TRUNCATE t` removes CH-side rows. Pre-phase-14
//! the decoder dropped TRUNCATE silently (no block ref) and CH retained
//! stale rows.
//!
//! Drill shape:
...
async fn phase14_truncate_removes_ch_rows() {
    if !fx::pg_available() {
