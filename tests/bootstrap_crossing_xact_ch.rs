//! Verify pump rebuilds transaction open across handoff from
...
async fn transaction_open_across_the_handoff_reaches_ch() {
    crossing_transaction(true).await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn transaction_open_across_the_handoff_reaches_ch_without_slot() {
    crossing_transaction(false).await;
}
...
async fn crossing_transaction(with_slot: bool) {
    ...
    let child = daemon
        .spawn(&source, &ch_config_path, slot.walsender, &args)
    ...
    let result: Result<()> = async {
        ...
        // Direct mode can fall back to landed WAL
        daemon.wait_for_log("backup window shipped", Duration::from_secs(90))?;
        ...
        // Require crossing transaction at window seal
