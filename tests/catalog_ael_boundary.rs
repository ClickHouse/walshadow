...
async fn run_boundary_ddl(held: Held<'_>, d_ddl: &str) -> Result<(), String> {
    ...
    source.psql_one(d_ddl).map_err(|e| format!("D ddl: {e}"))?;
    let target = {
        let s = source
...
/// Fast-default sub-case: `DEFAULT k` sets `attmissingval`, and rendering it via
/// `anyarray_out` would re-take the recovery-held `pg_type` lock. Worker must
