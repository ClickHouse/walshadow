//! Phase 14 — bootstrap + CH end-to-end via the object-store
//! BASE_BACKUP source.
//!
//! Sibling of `phase14_bootstrap_direct_ch.rs` covering the wal-g
//! layout. Setup adds a `wal_rs::pg::backup::push::handle` call between
//! source workload load and daemon spawn — same fixture pattern as
//! `phase12_object_store_e2e.rs`, just with a live CH server + real
...
//!   → wal_rs::pg::backup::push::handle → FsStorage(wal-g/)
...
/// `phase12_object_store_e2e.rs::test_settings`.
...
async fn object_store_bootstrap_ch_end_to_end() {
    ...
    //    via `wal_rs::config::Settings::from_env()`. WALG_FILE_PREFIX
