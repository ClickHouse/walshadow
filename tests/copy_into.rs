//! `XLOG_HEAP2_MULTI_INSERT` per-tuple fan-out.
//!
//! Verifies that `COPY foo FROM stdin` rows reach CH. Earlier decoder
//! returned `Ok(None)` for every Heap2 multi-insert record,
...
async fn copy_into_multi_insert_replicates() {
    ...
    let mappings = vec![fx::TableMappingSpec {
        source_table: RelName::new("s14", "copy_t"),
        target_table: TableTarget::new("walshadow_test", "s14_copy_t"),
        columns: vec![
            ColumnMapping {
                src_attnum: 1,
                target_name: "id".into(),
