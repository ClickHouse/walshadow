//! Phase 12 — file-streaming bootstrap end-to-end via the
//! object-store source.
//!
...
//!   on-disk HeapTupleHeader reshape into xl_heap_header for the Phase 5
//!   decoder)
...
//! oracle is ever wanted; PHASE12 doesn't depend on it. This test runs
...
/// Port slot reserved for Phase 12 (56140-range). Single source-PG per
...
fn append_source_conf(sh: &Shadow) {
    ...
    writeln!(f, "\n# walshadow Phase 12 source overrides").unwrap();
