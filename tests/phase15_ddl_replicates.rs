//! PHASE15 §4 + §5 + §6 — DDL replicates end-to-end.
//!
//! Three drills:
//!
//! 1. `phase15_alter_add_column_replicates_without_toml_edit` (§4)
...
//!
//! 2. `phase15_create_table_auto_replicates_in_namespace` (§5)
...
//!
//! 3. `phase15_drop_table_strategy_drop_removes_dest` (§6)
...
async fn phase15_alter_add_column_replicates_without_toml_edit() {
    if !fx::pg_available() {
    ...
    let mut pipeline = fx::build_pipeline(fx::BuildPipelineArgs {
        ...
        mappings,
        app_name: "walshadow-phase15-alter-add",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn phase15_create_table_auto_replicates_in_namespace() {
    ...
    let mut pipeline = fx::build_pipeline(fx::BuildPipelineArgs {
        ...
        ch_tcp_port,
        mappings: vec![],
        app_name: "walshadow-phase15-create-auto",
        ddl: Some(ddl_args),
    })
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn phase15_drop_table_strategy_drop_removes_dest() {
    if !fx::pg_available() || !fx::pg_basebackup_available() || !fx::clickhouse_available() {
        eprintln!("skip: missing initdb / pg_basebackup / clickhouse");
        return;
    }
    ...
    let mut pipeline = fx::build_pipeline(fx::BuildPipelineArgs {
        ...
        ch_tcp_port,
        mappings: vec![],
        app_name: "walshadow-phase15-drop",
        ddl: Some(ddl_args),
    })
    .await;
