//! Phase 14 item 08 — v1.0 acceptance §1: pgbench workload intermixed
//! with `ALTER TABLE ... ADD COLUMN ... DEFAULT k` (fast-path) and
//! `CREATE INDEX CONCURRENTLY`, end-state parity source vs CH.
//!
//! Pipeline mirrors `phase14_bootstrap_direct_ch.rs`:
//!
...
/// synthetic `_lsn`/`_xid`/`_op`/`_commit_ts` trailer Phase 7 emits.
